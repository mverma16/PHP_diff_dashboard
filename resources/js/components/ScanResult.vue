	    ...
	    <div v-if="scanResultData">
		    ...
		    <div v-if="scanResultData.data.file_updated.length" class="row">
			    <div class="col-md-12">
				    ...
				    <ul class="nav nav-tabs">
					   <li role="presentation" :class="[showList ? 'active':'']" data-toggle="tooltip" title="View as updates in base version">
                            <a v-on:click=toggleShowList(true)><span class="glyphicon glyphicon-th-list"></span></a>
                        </li>
  					    <li role="presentation" :class="[!showList ? 'active':'']" data-toggle="tooltip" title="View as comparison in both versions">
                            <a v-on:click=toggleShowList(false)><span class="glyphicon glyphicon-th"></span></a>
                        </li>
				    </ul>
				    <div v-if="showList">
  					    <ul class="list-group">
		    ...
		    <div v-if="scanResultData.data.file_changed.length" class="row">
			    <div class="col-md-12">
                    ...
                    <router-link :to="{path: '/modified-files/' + scanResultData.data.id}" data-toggle="tooltip" title="Click to view diff in all modified files of this scan">Show Complete Diff</router-link>
				    <ul class="list-group">
					    <li v-for="file in scanResultData.data.file_changed" class="list-group-item list-group-item-warning">
                            <router-link :to="{path: '/file-diff/' + file.id}" >
  							    <span class="glyphicon glyphicon-file"></span> {{file.file_path}}<span class="glyphicon glyphicon-eye-open pull-right" data-toggle="tooltip" title="click to view diff in the file"></span>
                            </router-link>
  					    </li>
				    </ul>
			    </div>
...
<script type="text/javascript">
	...
	export default {
        ...
        beforeMount() {
            this.resultType = this.$route.name;            
            if('id' in this.$route.params) {
                this.getScanResult(this.$route.params.id)
            } else {
                this.fetchResults();
            }
        }
